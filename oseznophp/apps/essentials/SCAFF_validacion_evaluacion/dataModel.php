 ...
 class scaffolding_evaluacion_indicadores {
	...
	{   
 	...
 	public static function getFormAddModevaluacion_indicadores($id){
		...
		$myForm->addTextArea('Compromisos:', 'descripcion', $evaluacion_indicadores->descripcion,90, 3);
                
                $myForm->addCheckBox('Validar Producto', 'a');  
 	...
 	public function getList_evaluacion_indicadores (){
 		...
 		$sql = 'SELECT prod.id_producto AS "Validación", prod.nombre_producto AS  "Nombre Producto", users.user_name AS  "Usuario", eva.peso AS  "Peso", eva.recurso AS  "Recurso", eva.proceso AS  "Proceso", eva.producto AS  "Producto", eva.cliente AS  "Cliente", eva.puntaje_ponderado AS "Puntaje Ponderado", eva.descripcion AS  "Descripción", eva.fecha_modificacion AS  "Fecha", eva.compromisos AS  "Compromisos", eva.validacion AS  "Validación"
FROM producto prod, evaluacion_indicadores eva, ess_system_users users
WHERE eva.id_producto = prod.id_producto'; //PENDIENTE COLOCAR EL FILTRO DE LA AÑOOOOO
		...
		$myList->setExportData(true,true,true);
 		...
 		$myList->setEventOnColumn('Validación','onClickAddRecord');
